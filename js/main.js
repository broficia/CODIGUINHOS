...
window.CURRENCIES=CURRENCIES;window.LANGS=LANGS;
...
function updateNavAuth(){
  ...
  // Inject Perfil + Indique e Ganhe items into the nav menu (mobile + desktop)
  if(navLinks && !navLinks.querySelector('[data-nav-extra="1"]')){
    const liProfile=document.createElement('li');
    liProfile.dataset.navExtra='1';
    liProfile.innerHTML=`<a href="${prefix}profile.html" id="nav-profile">👤 PERFIL</a>`;
    const liPromo=document.createElement('li');
    liPromo.dataset.navExtra='1';
    liPromo.innerHTML=`<a href="${prefix}promo.html" id="nav-promo" style="color:#ffd84d">🎁 INDIQUE E GANHE</a>`;
    navLinks.appendChild(liProfile);
    navLinks.appendChild(liPromo);
  }
  if(!el)return;
  if(user){
